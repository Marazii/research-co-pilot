...
- **The workspace root moved from the hidden `.research/` to a visible `research/<project-slug>/` folder** with a numbered, lifecycle-stage template:
  ```
  research/<project>/
    ...
    audits/  archive/
  ```
  The knowledge notes moved from `.research/vault/` to `research/<project>/knowledge/`. The manifest gains a `stage` field per artifact and lives at the project root. Audit reports land in `audits/`; superseded files go to `archive/`.
- **Every skill now files its output into the matching stage folder** (methodology → `03-methodology/`, analysis → `07-analysis/`, drafts → `08-drafts/`, review → `09-review/`, talks → `10-dissemination/`, etc.) — see the stage→folder map in `docs/research-vault.md`. Each skill's `## Handoffs` "Output to the vault" line names its stage folder.
- **Auto-generated root `README.md`** — the project's human-facing front page (title, current stage, facts table, contents map, last audit), regenerated by `/vault`.

### Added

- **`/vault init`** now scaffolds the full visible template (all stage folders + `knowledge/` + `audits/` + `archive/` + manifest + root README), with PII reminders in `entities.md` and `06-data/`.
- **`/vault organize`** — a new subcommand that tidies the project: files loose artifacts (from the root or `00-inbox/`) into their correct stage folder, moves superseded versions to `archive/`, and regenerates the root README. Moves files; never edits content.
- **Seventh audit check — filing:** `/vault audit` now also flags artifacts sitting in the wrong stage folder or loose in the root, and offers `/vault organize`.
- **`/research` conductor** initializes the project vault from the template, offers `organize` when files drift loose, runs the `/vault audit` gate before pre-submission, and reports the contents map + facts in the pipeline status.

### Design (unchanged from v0.11.1, reaffirmed)

- The vault never holds PII (pseudonyms only; `06-data/` is anonymized data only; the real-name key stays off-system; the audit scans for leaks).
- Files are the source of truth; the vault is reconciled — `organize` and `audit` move/report, never rewrite a document's content.
- Distinct facts over overwrites; flag-on-write on genuine conflicts only.
- Opt-in — standalone skill runs are unchanged.

### Notes for upgraders

- Existing `.research/` projects: the new model uses `research/<project>/`. There's no automatic migration; run `/vault init` for a new project, or move your `.research/` files into the new template (`/vault organize` will help file them once they're under `research/<project>/`).
- claude.ai: the vault is the folder in the sandbox; invoke `vault` by name; re-upload the project folder across sessions.

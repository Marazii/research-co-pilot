...
[![Version](https://img.shields.io/badge/version-0.8.0-blue.svg)](https://github.com/Marazii/research-co-pilot/blob/main/CHANGELOG.md)
...
| Getting feedback | **peer-review** | "review my paper", "fact-check this draft", "review my slides for the conference talk" |
...
| `manuscript-drafter` | Drafts long-form manuscript sections (abstract, intro, related work, methods, results, discussion, limitations, conclusion) from your methodology + analysis + existing manuscript. **Extracts the manuscript's voice profile** and applies it to every new paragraph. Hard per-section word budgets, multi-pass refinement (compression / register audit / voice-consistency check), and an academic-register guard against colloquialisms and conversational openers. Distinguishes `[CITATION NEEDED]` (known claim, source missing) from `[LITERATURE NEEDED]` (skill-introduced new idea needing grounding — with concrete search guidance). Drafts in the language of the existing manuscript (English, Hebrew, etc.). |
